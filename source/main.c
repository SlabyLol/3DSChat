...
static void send_page(int fd)
{
    ...
    if (g_msg_count == 0) {
        const char *empty =
            "<p style='color:#556;margin:auto'>"
            "Noch keine Nachrichten. Sag Hallo! &#128075;</p>";
        write(fd, empty, strlen(empty));
    }

    for (int i = 0; i < g_msg_count; i++) {
        char safe_name[NAME_LEN * 6];
        char safe_text[MSG_LEN  * 6];
...
static void handle_client(int fd)
{
    ...
    if (strlen(body) < 4) {
        int extra = (int)recv(fd, buf + total, (size_t)(RECV_BUF - 1 - total), 0);
        if (extra > 0) {
            total += extra;
            buf[total] = '\0';
            body = strstr(buf, "\r\n\r\n");
